  ...
  const statsFetches = liveMatches.map(async m => {
    ...
    try {
      ...
      const enriched: NonNullable<LiveMatch['sofascore_stats']> = {
        ...
        // Additional stats for complete coverage
        homeShotsOff: s.homeShotsOff ?? 0,
        awayShotsOff: s.awayShotsOff ?? 0,
        homeGoalsPrevented: s.homeGoalsPrevented ?? 0,
        awayGoalsPrevented: s.awayGoalsPrevented ?? 0,
        homeShotsOnTarget: s.homeSoT ?? 0,
        awayShotsOnTarget: s.awaySoT ?? 0,
        homeTotalShots: s.homeShots ?? 0,
        awayTotalShots: s.awayShots ?? 0,
        homeCorners: s.homeCorners ?? 0,
        awayCorners: s.awayCorners ?? 0,
        homePossession: s.homePoss ?? 0,
        awayPossession: s.awayPoss ?? 0,
        homeOffsides: s.homeOffsides ?? 0,
        awayOffsides: s.awayOffsides ?? 0,
        homeYellowCards: s.homeYellow ?? 0,
        awayYellowCards: s.awayYellow ?? 0,
        homeRedCards: s.homeRed ?? 0,
        awayRedCards: s.awayRed ?? 0,
        fetchedAt: now,
      };
